...
const SignIn = () => {
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(235,239,255,0.9),_transparent_34%),_linear-gradient(180deg,#f5f7ff_0%,#eef3ff_100%)] pb-24">
      <div className="mx-auto flex min-h-screen w-full max-w-3xl flex-col px-4 pt-10 md:px-8 lg:px-12">
        <div className="mb-8 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-600">Login with your name and mobile number.</p>
          </div>
        </div>
        ...
        <div className="rounded-[40px] bg-white/95 p-8 shadow-2xl backdrop-blur-sm md:p-12">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full rounded-full bg-gradient-to-r from-sky-500 to-indigo-600 px-6 py-4 text-lg font-semibold text-white shadow-xl transition hover:opacity-95 disabled:cursor-not-allowed disabled:opacity-50"
            >
          ...
          <p className="mt-6 text-center text-sm text-slate-600">
            Naya user?{' '}
            <Link to="/signup" className="font-semibold text-sky-600 hover:underline">Create account</Link>
          </p>
        </div>
      </div>
    </div>
