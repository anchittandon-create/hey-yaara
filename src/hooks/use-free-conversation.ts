...
export const useFreeConversation = (options: UseConversationOptions): ConversationSession => {
  ...
  const generateAssistantMessage = useCallback(
    async ({
      ...
      try {
        const response = await fetch("/api/yaara-chat", {
          method: "POST",
          headers: {
        ...
        if (!response.ok) {
          ...
          throw new Error(`Yaara chat API error: ${response.status} ${errorText}`);
        }

        const result = await response.json();
        const assistantMessage = result?.content?.trim() ?? null;
